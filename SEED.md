...
- A Hermes profile (default name `daniel`) MUST exist on the scaffold and MUST have its `model.provider` + `model.default` mirrored from the scaffold-level config (per `seed-hermes-gbrain` `^act-profile-model-mirror`). ^obj-owner-profile
...
- A SECOND Hermes profile (default name `daniel-team`) MUST exist on the scaffold and MUST have its `model.provider` + `model.default` mirrored from the scaffold-level config. ^obj-team-profile
- The profile MUST have a `plow_chat` gateway platform enabled and MUST be configured with the multi-token env `PLOW_CHATS=<uid1>:<key1>,<uid2>:<key2>,…` listing every team member's chat. v0.1.0 REQUIRES the patched `seed-hermes-plow-chat` adapter that consumes this env (Slack-style multi-token precedent). ^obj-team-plow-chats-env
