...
echo "== seed =="
# stay_status set explicitly for determinism (independent of the DTU server clock).
post /admin/reservation '{"property":"mtn-home","reservation_code":"R-MID","status":"accepted","stay_status":"in_house","check_in_date":"2026-05-20","check_out_date":"2026-05-26","conversation_id":"C-MID","guest_name":"Alice","number_of_guests":2}'
post /admin/reservation '{"property":"mtn-home","reservation_code":"R-FUT","status":"accepted","stay_status":"checkin_pending","check_in_date":"2026-06-10","check_out_date":"2026-06-13","conversation_id":"C-FUT","guest_name":"Frank"}'
post /admin/block '{"property":"mtn-home","start_date":"2026-06-01","end_date":"2026-06-03","remarks":"deep clean"}'
post /admin/reservation '{"property":"10th-ave","reservation_code":"R-ARR","status":"accepted","stay_status":"checkin_pending","check_in_date":"2026-05-25","check_out_date":"2026-05-28","conversation_id":"C-ARR","guest_name":"Arno"}'
post /admin/reservation '{"property":"10th-ave","reservation_code":"R-PAST","status":"accepted","stay_status":"stay_completed","check_in_date":"2026-04-01","check_out_date":"2026-04-05","conversation_id":"C-PAST","guest_name":"Pat"}'
...
# Real-world quirk: physically in_house while the calendar still says "future" (early check-in).
post /admin/reservation '{"property":"10th-ave","reservation_code":"R-EARLY","status":"accepted","stay_status":"in_house","check_in_date":"2026-06-15","check_out_date":"2026-06-18","conversation_id":"C-EARLY","guest_name":"Erin"}'
echo "seeded"
...
check "stay_status-surfaced" "$(hx guest-state --conversation C-MID)" "d['reservation']['stay_status']=='in_house'"
check "early-checkin-override" "$(hx guest-state --conversation C-EARLY)" "d['state']=='checked_in_midstay' and d['reservation']['stay_status']=='in_house'"
...
check "res-next-bookings" "$(hx reservations --property mtn-home --upcoming --limit 1)" "d['count']==1 and d['reservations'][0]['reservation_code']=='R-FUT'"
...
check "res-shape"   "$(curl -fsS "$DTU_BASE/v3/reservations?property_id=12051776&status=accepted")" "len(d['data']['reservations'])>=1 and 'check_in_date' in d['data']['reservations'][0] and 'stay_status' in d['data']['reservations'][0] and 'total' not in d['data']"
